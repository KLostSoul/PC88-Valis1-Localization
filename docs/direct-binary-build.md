...
이 프로젝트의 산출물은 어셈블러로 새 프로그램을 조립한 것이 아닙니다. 디버거로 원본 프로그램을 역어셈블하고 실행 경로를 확인한 뒤, 실제 수정이 필요한 위치를 확정하여 Python이 원본 복사본에 리터럴 바이트를 직접 기록합니다.
...
분석 근거가 있다고 해서 빌드 시점에 자동 추출·자동 매핑·주소 추정이 수행되는 것은 아닙니다. 확정된 소스 행만 빌드에 참여합니다.
...
이 단계는 트랙·섹터·payload 구조를 읽기 전용으로 보고할 뿐이며, 번역 행이나 패치 행을 생성하지 않습니다.
...
완료본의 실제 수정은 역어셈블·디버거 확인 뒤 바이너리 위치를 직접 수정한 방식으로 확정되어 있습니다. 이 저장소는 그 작업 방식을 재현하기 위해 ASM 파일을 관찰 기록으로만 보관합니다. ASM을 다시 조립하는 단계, 오브젝트 파일, 링크 스크립트, 후킹 코드 생성기는 없습니다.

## 6. 저작권 자료 경계

완료본 ZIP, 완료 D88/ROM, IPS, 원본 D88/ROM, 추출 이미지와 같은 파일은 저장소에 넣지 않습니다. 사용자는 자신의 로컬 경로를 명령행 인자로 전달합니다. `compare`만 로컬 비교 파일을 읽을 수 있고, `build`는 완료본·IPS를 인자로 받지 않습니다.
