...
원본 Disk A D88과 원본 KANJI1 ROM을 입력으로 받아 검토된 직접 바이트 표를 적용하고, 파일 구조와 최종 해시를 검증합니다. 텍스트 컴파일러나 자동 추출기가 아닙니다.

## 입력과 출력

- 입력: 사용자가 제공한 원본 D88과 KANJI1 ROM
- 출력: 원본 컨테이너를 보존한 재현 D88과 수정된 KANJI1 ROM
- 비교 자료: 완료본·IPS·추출 이미지. 빌드 입력이나 저장소 원천이 아닙니다.
...
이벤트 1~6, 게임오버, 엔딩 24개 세그먼트, 로고, ERROR 07, 제어 토큰, KANJI1 글리프와 직접 기록기가 각각 명시된 원천표를 사용합니다. ASM은 디버거 관찰 기록이며 조립하지 않습니다.
