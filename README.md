...
이 저장소의 자체 제작 코드, 빌드 스크립트, 테스트 코드는 MIT License로 배포합니다.

문서, 분석 자료, 빌드 메타데이터, 자체 제작 표와 기타 프로젝트 고유 자료는 **출처를 명확히 밝히는 조건으로 수정·재배포할 수 있습니다.** 별도의 허가를 받을 필요는 없습니다.

재배포하거나 수정본을 공개할 때는 다음 출처를 표시해야 합니다.

```text
PC88-Valis1-Localization-Build by KLostSoul
https://github.com/KLostSoul/PC88-Valis1-Localization-Build
```

수정본은 원본 프로젝트에서 변경된 버전임을 함께 표시해야 합니다. 자세한 적용 범위는 [LICENSE](LICENSE)를 확인하세요.

다만 원작 게임, 원본 ROM/D88 내용, 원작 텍스트·캐릭터·이미지·음악·상표 등 제3자 저작물에 대한 권리를 이 저장소가 부여하는 것은 아닙니다. 번역·분석·패치 자료 역시 이 프로젝트가 보유한 기여분의 권리에 한해서만 위 허가가 적용됩니다.

원본 상용 게임의 D88/ROM 이미지는 이 저장소의 라이선스로 배포되지 않으며, 빌드에 필요한 원본 미디어는 사용자가 적법하게 준비해야 합니다.

## 주의
이 빌드는 비공식 한글 패치이며, 원작에 관한 모든 권리는 각 권리자에게 있습니다.
