...
빌드는 사용자가 제공한 원본 D88과 `KANJI1.ROM`을 읽고, 분석과 디버거 확인을 거쳐 확정한 직접 바이트 패치표를 원본 이미지에 적용합니다.

원본 이미지는 `QUASI88 0.7.4`의 디버그 기능을 사용해서 분석했고, 여타 다른 현지화 패치와 달리 직접 디스크 섹터의 데이터를 수정하는 방식으로 패치를 진행했습니다.
